describe('inputDirective', function () {
  ...
  beforeEach(inject(function ($rootScope, $compile) {
    ...
    it('valid date value in ng-model should set $valid to false', function () {
      scope.myDate = new Date(2014, 0, 1);
      recompile();
      expect(scope.inputForm.$valid).toBe(true, 'valid date');
    });

    // NOTE(c0bra): This fails with angular 1.3.0. No idea why. Do we need it? Turning off for now.
    // it('invalid date value in ng-model should set $valid to false', function () {
    //   scope.myDate = new Date('invalid date');
    //   recompile();
    //   expect(scope.inputForm.$valid).toBe(false, 'invalid date');
    // });

    it('null date value in ng-model should set $valid to true', function () {
      scope.myDate = null;
      recompile();
      expect(scope.inputForm.$valid).toBe(true, 'null value');
    });
});