...
var util = module.exports = {
  ...
  testDependencies: {
    unit: ['bower_components/jquery/jquery.min.js', 'lib/test/jquery.simulate.js', 'dist/release/ui-grid.css']
  },
  ...
  isStable: function(version) {
    // return semver.satisfies(version, '2.0 || 3.0') && version.prerelease.length === 0;
  ...
  getPreviousVersions:  function() {
    ...
    var repo_url = util.getPackage().repository.url;
    var tagResults = shell.exec('git ls-remote --tags ' + repo_url,
                                {silent: true});
    if ( tagResults.code === 0 ) {
      return _(tagResults.output.match(/v[2-9].*[0-9]$/mg))
        .map(function(tag) {
        ...
        .map(function(version) {
          version.isStable = util.isStable(version);
  ...
  getGitRepoInfo: function() {
    ...
    var match = GITURL_REGEX.exec(util.getPackage().repository.url);
    var git = {
      owner: match[1],
      repo: match[2]
    };
